...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Saídas</h1>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="border-2">
                        <div class="flex items-center justify-center space-x-4">
                            <button class="text-[10px] md:text-xl" title="Editar" onclick="window.location.href = '{{ route('saidas.edit', $saida->id) }}'"><i class='bx bxs-edit'></i></button>
                            <form method="POST" action="{{route('saidas.destroy', $saida->id)}}">
                                @method('delete')
                                @csrf
    ...
    @section('modal-title')
        <h2 class="flex justify-items-start items-center">
            ...
            <label>Adicionar uma nova Saída</label>
        </h2>
    @endsection
    @section('modal-content')
        <div class="w-full mx-auto p-4 overflow-y-auto h-5/6">
            <form id="saidaForm" method="POST" action="{{route('saidas.store')}}">
                @csrf()
                <div class="w-full p-1 gap-4 flex flex-wrap md:flex-nowrap items-center">
                    <div class="w-full md:w-1/2 mb-4">
                        <label for="saida_descricao" class="block text-sm font-medium text-gray-700">Descrição da Saída</label>
                        <input type="text" name="saida_descricao" id="saida_descricao" value="{{ $saida_edit['saida_descricao'] }}" class="w-full border-gray-300 rounded-md p-2" placeholder="Digite a descrição da saída" required>
                    </div>
                    <div class="w-full md:w-1/2 mb-4">
                        <label for="saida_data" class="block text-sm font-medium text-gray-700">Data da Saída*</label>
                        <input type="date" name="saida_data" id="saida_data" value="{{ $saida_edit['saida_data'] }}" class="w-full border-gray-300 rounded-md p-2" placeholder="00/00/0000" required>
                    </div>
                </div>
                ...
                <div class="w-full p-1 gap-4 flex flex-wrap md:flex-nowrap items-center">
                    <div class="w-full md:w-1/3 mb-4">
                        <label for="saida_valor" class="block text-sm font-medium text-gray-700">Valor Total*</label>
                        <input type="text" name="saida_valor" id="saida_valor" value="{{ $saida_edit['saida_valor'] }}" class="w-full border-gray-300 rounded-md p-2" placeholder="R$0.000.000,00">
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if(resultEdit === 'edit'){
                ...
                const formulario = document.getElementById('saidaForm');
                ...
                if (botaoAdicionar && formulario) {
                    ...
                    formulario.action = '{{ route('saidas.update',$saida_edit['id']) }}';
                    ...
                    const motoristaId = '{{ $saida_edit['motorista_id'] }}';
                ...
                // tela deve voltar a tela inicial de saidas
                document.getElementById('fechar-modal').addEventListener('click', function () {
                    window.location.href = '{{ route('saidas.saidas') }}';
                });
        }
